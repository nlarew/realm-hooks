...
const Test = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <RequireLoggedInUser app={app} fallback={<button onClick={handleLogin}>Log In</button>}>
      <React.Fragment>
        <div>logged in</div>
...
describe("RequireLoggedInUser", () => {
  ...
  test("it renders its children if a user is logged in", async () => {
    ...
    expect(screen.getByRole("heading").textContent).toBe("If you see this, you are logged in")
  })
})
